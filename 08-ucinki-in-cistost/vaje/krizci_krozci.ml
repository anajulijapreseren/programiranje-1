...
(*ker v ocamlu ne moremo indeksirati tako kot v Pythonu*)
(*----------------------------------------------------------------------------*]
...
(*let is_full_row (x1, x2, x3) = if (x1 = None or x2 = None or x3 = None)*)
...
let place_token player grid (row_i, col_i) : state =
  ...
  (*pazi:tu še ne preverimo, ali je mesto prazno*)
...
let show_row (a0, a1, a2) =
  ...
  ^ " |\n"
(*.concat isto kot v pythonu .join*)
...
let rec loop state = 
  ...
  | OnTurn {player; grid} -> (
    ...
    (*preveriti moramo ali je mesto frej*)
